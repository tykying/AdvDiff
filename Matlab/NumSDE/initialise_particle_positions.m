...
for j_cell = 1: ncell_DIR(2)
    for i_cell = 1: ncell_DIR(1)
        % Within each cell
        for j_part = 1: npart_pc_DIR(2)
            for i_part = 1: npart_pc_DIR(1)
                nparticles_counter = nparticles_counter + 1;
                
                x0(nparticles_counter) = (i_cell-1)*cellsize_DIR(1) + (i_part - 0.5)*partsep_DIR(1) +PART_ranges_min(1);
                y0(nparticles_counter) = (j_cell-1)*cellsize_DIR(2) + (j_part - 0.5)*partsep_DIR(2) +PART_ranges_min(2);
            end
        end
    end
end
